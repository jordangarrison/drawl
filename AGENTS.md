...
- **Walker uses multimethod dispatch on head symbol** (`defmulti walk-form`). Macros — both built-in (`webapp`, `postgres-db`, `redis-cache`, `rest-api`) and user-defined `(defmacro ...)` — share one mechanism: a per-compile registry on `(:macros ctx)`. The `:default` defmethod consults the registry, binds args to params, substitutes into the template body, and recurses on the expansion. Built-ins are seeded from `drawl.macros/builtins`; user `defmacro` forms appear at the top level (outside the diagram) and override built-ins with a warning. No `eval`, no `~`/`~@` reader markers — just symbol-leaf substitution with `&` capturing the rest as a splice-site.
...
- **PWA cache invalidation**: `public/sw.js` is generated at build time from `public/sw.js.tmpl`. The `build-hooks.sw-stamp` shadow-cljs hook substitutes `__CACHE_VERSION__` with `drawl-v<package.json version>-alpha-<sha256(main.js)[0..6]>`, so cache keys auto-rotate whenever the bundle content changes. Don't edit `public/sw.js` directly — it's gitignored. Edit the template, or change the hook in `src/build_hooks/sw_stamp.clj`.
